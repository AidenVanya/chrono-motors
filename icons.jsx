...
/* ---- Chronograph emblem logo ---- */
function ChronoMark({ size = 40, stroke = "var(--gold)", spin = true }) {
  const ticks = [];
  for (let i = 0; i < 60; i++) {
    const major = i % 5 === 0;
...
function Logo({ light = false, size = 38 }) {
  return (
...
/* ============================================================
   ...
   props: c1, c2 (gradient body colors), className, glow
   ============================================================ */
...
/* Each silhouette returns the BODY path(s); wheels added by CarSilhouette */
const BODIES = {
  coupe: {
...
function CarSilhouette({ type = "coupe", c1 = "#d61f26", c2 = "#7a0d12", className = "", uid = "x", showGlass = true }) {
...
const IconPower   = (p) => <Icon {...p} d="M13 2 3 14h7l-1 8 10-12h-7l1-8z" />;
const IconGauge   = (p) => <Icon {...p}><path d="M12 14l4-4" /><circle cx="12" cy="13" r="9" /></Icon>;
const IconTop     = (p) => <Icon {...p} d="M3 17l6-6 4 4 8-8M21 7v6h-6" />;
...
const IconPhone   = (p) => <Icon {...p} d="M22 16.9v3a2 2 0 0 1-2.2 2 19.8 19.8 0 0 1-8.6-3 19.5 19.5 0 0 1-6-6 19.8 19.8 0 0 1-3-8.6A2 2 0 0 1 4.1 2h3a2 2 0 0 1 2 1.7c.1 1 .4 1.9.7 2.8a2 2 0 0 1-.5 2.1L8.1 9.9a16 16 0 0 0 6 6l1.3-1.2a2 2 0 0 1 2.1-.5c.9.3 1.8.6 2.8.7a2 2 0 0 1 1.7 2z" />;
const IconMail    = (p) => <Icon {...p}><rect x="2" y="4" width="20" height="16" rx="2" /><path d="m22 7-10 6L2 7" /></Icon>;
const IconPin     = (p) => <Icon {...p}><path d="M21 10c0 6-9 12-9 12s-9-6-9-12a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></Icon>;
...
const IconSwap    = (p) => <Icon {...p} d="M7 4 3 8l4 4M3 8h13M17 20l4-4-4-4M21 16H8" />;
const IconStar    = (p) => <Icon {...p} d="M12 3l2.6 5.3 5.9.9-4.3 4.1 1 5.8-5.2-2.7-5.2 2.7 1-5.8L3.5 9.2l5.9-.9L12 3z" />;
const IconWallet  = (p) => <Icon {...p}><rect x="3" y="6" width="18" height="13" rx="2" /><path d="M16 12h3M3 9h18" /></Icon>;
const IconMenu    = (p) => <Icon {...p} d="M4 7h16M4 12h16M4 17h16" />;
const IconHeart   = (p) => <Icon {...p} d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.6l-1-1a5.5 5.5 0 0 0-7.8 7.8l1 1L12 21l7.8-7.6 1-1a5.5 5.5 0 0 0 0-7.8z" />;
const IconTrash   = (p) => <Icon {...p}><path d="M3 6h18M8 6V4h8v2M19 6l-1 14H6L5 6" /></Icon>;

Object.assign(window, {
  ChronoMark, Logo, CarSilhouette, Wheel,
  IconPower, IconGauge, IconTop, IconCal, IconArrow, IconArrowL, IconClose,
  IconPhone, IconMail, IconPin, IconClock, IconCheck, IconGlobe, IconShield,
  IconSwap, IconStar, IconWallet, IconMenu,
  IconHeart, IconTrash,
});
