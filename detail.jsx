...
const HOTSPOTS = [
  { x: "70%", y: "60%", i: 0 }, // engine (rear)
  { x: "46%", y: "40%", i: 1 }, // interior (cabin)
  { x: "30%", y: "74%", i: 2 }, // body (lower)
  { x: "84%", y: "56%", i: 3 }, // aero (rear)
];

function CarDetail({ car, t, lang, openCar, back }) {
  const [active, setActive] = useState(null);
  const [view, setView] = useState(0);
  if (!car) return null;

  // gallery "views" = same photo, different crop/zoom → multi-shot feel
  const views = [
...
Object.assign(window, { CarDetail });
