...
function Nav({ page, go, lang, setLang, t, openGarage, heroDone }) {
  ...
  // on home, stay transparent over the rotating hero until the rotation fully completes
  const onDark = page === "home" && !heroDone;
    ...
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${
      ...
      {open && (
        <div className="fixed inset-0 z-[60] md:hidden" onClick={() => setOpen(false)}>
          <div className="absolute inset-0 bg-night/70 backdrop-blur-sm" style={{ animation: "x-fade 0.3s both" }} />
          <div className="absolute top-0 right-0 bottom-0 w-[80%] max-w-xs bg-bg p-6 flex flex-col"
               style={{ animation: "reveal-up 0.35s var(--ease) both" }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-8">
              <Logo size={32} />
              <button onClick={() => setOpen(false)} className="text-ink"><IconClose size={24} /></button>
            </div>
            <nav className="flex flex-col gap-1">
              {links.map(([id, label]) => (
                <button key={id} onClick={() => { go(id); setOpen(false); }}
                  className={`text-left font-display text-2xl py-3 border-b border-line ${page === id ? "text-gold" : "text-ink"}`}>
                  {label}
                </button>
              ))}
            </nav>
            <button onClick={() => { go("contact"); setOpen(false); }}
              className="mt-auto inline-flex items-center justify-center gap-2 bg-ink text-white font-medium px-6 py-3.5 rounded-full">
...
function App() {
  ...
  // primary (canvas scrub) signal — fires while hero is scrubbing
  const onHeroProgress = (p) => setDone(p >= 0.96);

  // deterministic scroll fallback so the navbar is ALWAYS solid (dark text) once
  // we've scrolled past the hero rotation — guarantees readable nav over light sections
  useEffect(() => {
    const onScroll = () => {
      if (page !== "home") return;
  ...
  return (
    <div>
      <ScrollProgress />
      <Nav page={page} go={go} lang={lang} setLang={setLang} t={t} openGarage={() => setGarageOpen(true)} heroDone={heroDone} />
      <main>
